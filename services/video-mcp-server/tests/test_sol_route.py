...
class SolRouteTest(unittest.TestCase):
 ...
 def test_sol_route_and_instance_persist_without_fallback(self):
  ...
  with self.assertRaises(TaskConflict):self.executor.generate(**self.kw,service='h3')
 def test_h3_default_remains_legacy_payload_and_digest(self):
  expected=self.executor._normalize(self.kw['project_id'],self.kw['model'],self.kw['prompt'],5,'9:16',self.kw['references'])[1]
