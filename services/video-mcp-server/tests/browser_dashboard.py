    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                page.get_by_test_id('token-input').fill('browser-test-token')
                page.get_by_test_id('connect-token').click()
                expect(page.get_by_test_id('tasks-view')).to_be_visible()
                expect(page.get_by_test_id('task-card')).to_have_count(27)
                ...
                expect(page.get_by_role('heading', name='连接 Video MCP')).to_be_visible()
                expect(page.get_by_test_id('token-input')).to_be_empty()
                page.get_by_test_id('token-input').fill('invalid-token')
                page.get_by_test_id('connect-token').click()
